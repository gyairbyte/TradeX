...
def _write_safe_artifacts(
    ...
    lines = [
        ...
        f"Bundle generation head: {decision.bundle_generation_head}",
...
def _generate_report(plan: DatasetPlan, decision: DatasetDecision, output_dir: Path) -> str:
    ...
    lines.extend([
        ...
        "## Next step",
        "",
        f"`{decision.next_assignment}` — implement session VWAP, opening-drive state, pullback/reclaim detector, and baselines with synthetic tests only. Development/validation/holdout evaluation belongs to `devin/intra-001-d-locked-study` under a separate, explicitly approved PR.",
        "",
        "---",
