...
def _resolve_headline_source(source: str | None) -> str:
    ...
    raise ProviderCapabilityError(
        f"Headline source '{s}' is not supported; only 'yahoo' is available"
    )
...
def _select_recent_headline(
    ...
    for raw in headlines:
        if not isinstance(raw, dict):
            continue
        published = _parse_headline_timestamp(
            raw.get("published") or raw.get("published_at") or raw.get("date")
        )
...
def fetch_catalyst_context(
    ...
    if requested_earnings_source:
        try:
            ...
            if historical_replay:
                ...
                error = ValueError(
                    "Earnings calendar is not point-in-time for historical as_of; "
            ...
            if actual_headline_source == "yahoo":
                ...
                if exc:
                    ...
                    if selected:
                        ...
                        headline_publisher = selected["publisher"]
    ...
    return GapCatalystContext(
        ...
        headline_publisher=headline_publisher,
