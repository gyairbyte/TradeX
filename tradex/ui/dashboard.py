...
def _options_status_container(status, label: str) -> None:
    """Render a source status box with warnings for the given options capability."""
    if status.error:
...
- **1–3x** — typical to slightly elevated
    ...
    if flow_report is not None:
        if flow_report.total_matches:
            st.success(
                f"{flow_report.total_matches} flow events found "
                f"(source: {flow_report.actual_source}; data_kind: {flow_report.data_kind.value if flow_report.data_kind else 'unknown'})"
            )
            st.dataframe(flow_report.results, use_container_width=True)
        elif flow_report.failures:
            st.warning(
                f"No matches; {len(flow_report.failures)} ticker(s) failed. "
                f"Status: {flow_report.status.value}"
            )
            with st.expander("Failures"):
                st.json(flow_report.failures)
        else:
            st.info(
                f"No true options-flow events found above {min_vol_oi}x Vol/OI ratio "
                f"(source: {flow_report.actual_source or options_source})."
            )
    ...
    if chain_report is not None:
        if chain_report.total_matches:
            st.success(
                f"{chain_report.total_matches} chain contracts found "
                f"(source: {chain_report.actual_source}; data_kind: {chain_report.data_kind.value if chain_report.data_kind else 'unknown'})"
            )
            st.dataframe(chain_report.results, use_container_width=True)
        elif chain_report.failures:
            st.warning(
                f"No matches; {len(chain_report.failures)} ticker(s) failed. "
                f"Status: {chain_report.status.value}"
            )
            with st.expander("Failures"):
                st.json(chain_report.failures)
        else:
            st.info(
                f"No chain activity found above {min_vol_oi}x Vol/OI ratio "
                f"(source: {chain_report.actual_source or options_source})."
            )
