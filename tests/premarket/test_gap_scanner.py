...
def test_get_prev_close_propagates_provider():
    ...
    as_of = datetime(2024, 1, 5, 8, 0, tzinfo=UTC)
...
def test_scan_gaps_propagates_provider():
    """scan_gaps passes provider and as_of to both price paths."""
    as_of = datetime(2024, 1, 5, 8, 0, tzinfo=UTC)
    with (
    ...
    mock_prev.assert_called_once_with("AAPL", provider="yahoo", as_of=as_of)
    mock_pre.assert_called_once_with("AAPL", provider="yahoo", as_of=as_of)
...
def test_get_premarket_price_summer_bars():
    ...
    times = pd.DatetimeIndex([
        "2024-07-03 08:00", "2024-07-03 09:00", "2024-07-03 10:00"
    ], tz="UTC")
    df = pd.DataFrame({
        "Open": [100.0, 101.0, 103.0],
        "High": [101.0, 102.0, 104.0],
...
def test_get_premarket_price_excludes_exact_open():
    ...
    times = pd.DatetimeIndex([
        "2024-01-03 14:00", "2024-01-03 14:30"
    ], tz="UTC")
    df = pd.DataFrame({
        "Open": [100.0, 103.0],
        "High": [101.0, 104.0],
...
def test_get_premarket_price_excludes_prior_after_hours():
    ...
    times = pd.DatetimeIndex([
        "2024-01-02 22:00", "2024-01-03 14:30"
    ], tz="UTC")
    df = pd.DataFrame({
        "Open": [99.0, 103.0],
        "High": [100.0, 104.0],
...
def test_get_premarket_price_excludes_before_4am():
    ...
    df = pd.DataFrame({
        "Open": [100.0], "High": [101.0], "Low": [99.0], "Close": [101.0], "Volume": [100],
    }, index=times)
