...
No `INTRA-001` detector, execution engine, backtester, or production scorer exists yet. The current production intraday score remains the four-component additive score described above. The `SHORT-001` Schwab data-quality issue is now resolved, so `INTRA-001B` (empirically probe Schwab multi-year five-minute history and lock the intraday data-source decision) is the recommended next research work. A separate Gary-approved production PR is required only if all gates pass.
