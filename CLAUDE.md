...
- **Rapid-upside long opportunity research (LONG-002)** — A separate, new research program (`docs/research/LONG-002.md` and `docs/research/specs/LONG-002-v1.json`) studies +10%/+20%/+30% moves over 5/10/21 sessions for U.S. mid-/large-/mega-cap common stocks. `LONG-002A` (locked contract) is merged; `LONG-002B` (core data feasibility) is `supported_with_documented_limitations` on a bounded provider probe. Earnings event timing is `not_supported`; a Gary-approved amendment is required before full dataset construction. `production_promotion_eligible=false`.
