...
- **Rapid-upside long opportunity research (LONG-002)** — A separate, new research program (`docs/research/LONG-002.md` and `docs/research/specs/LONG-002-v1.json`) studies +10%/+20%/+30% moves over 5/10/21 sessions for U.S. mid-/large-/mega-cap common stocks. `LONG-002A` (locked contract) is merged; `LONG-002B` (core data feasibility) is `not_supported` overall on a bounded provider probe (daily market data and fundamentals/shares are `supported_with_documented_limitations`; security master & corporate actions and earnings event timing are `not_supported`). No live calls were made for earnings; preregistered candidates remain unverified. A Gary-approved amendment is required before `LONG-002C` full dataset construction. `production_promotion_eligible=false`.
