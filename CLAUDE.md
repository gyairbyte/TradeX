...
- **Rapid-upside long opportunity research (LONG-002)** — A separate, new research program (`docs/research/LONG-002.md` and `docs/research/specs/LONG-002-v1.json`) studies +10%/+20%/+30% moves over 5/10/21 sessions for U.S. mid-/large-/mega-cap common stocks. `LONG-002A` (locked contract) and `LONG-002B` (core data feasibility) are merged. `LONG-002B-AMEND-001` resolves the security-identity/lifecycle/exclusion family to `supported_with_documented_limitations` using Massive singular ticker details, the ticker-type taxonomy, ticker-change events, and split/dividend endpoints. Earnings-event timing remains `not_supported`; the amendment records a fail-closed decision memo and does not authorize `LONG-002C`. `production_promotion_eligible=false`.
