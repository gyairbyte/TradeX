...
- **Product consolidation (MVP-ARCH-001)** — `docs/product/MVP-ARCH-001.json` is the authoritative inventory and `docs/product/MVP-ARCH-001.md` is the human-readable summary. All final decisions remain `pending_gary_decision`; the packet does not implement consolidation, remove providers, change dashboard tabs, alter alerts, migrate data, or begin `LONG-002C`. It is a separate workstream from `LONG-002`; `LONG-002B-AMEND-002` is completed and `LONG-002C` design is authorized by PR #52 but paused by Gary while MVP-ARCH-001 is completed.
