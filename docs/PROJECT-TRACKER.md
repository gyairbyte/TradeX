...
- **Priority:** Medium
- **Status:** `INTRA-001D` complete on `devin/intra-001-d-locked-study` after review fixes — the holdout ledger was redesigned to a canonical per-dataset location that stores the full frozen identity (evaluation code, spec, amendment, and dataset plan SHAs), atomically writes a `started` tombstone before any holdout Parquet parse, and blocks any `started`/`completed`/`not_run` state plus any changed identity; the `--holdout-ledger-dir` option was removed; `generated_at_fixed` semantics were corrected so an explicit `--generated-at` is recorded as fixed and the synthetic default is not; adapter/firewall tests were expanded to cover `invalid`/`not_supported` outcome paths with measured zero holdout parses, monthly PIT inclusion/exclusion, same-ticker cross-month aggregation, deterministic whole-bundle equality, and network/credential isolation; the locked `INTRA-001B-DATASET-V1` snapshot was integrity-verified and development/validation were rerun from a clean freeze with no live provider calls; the study returned `inconclusive`; holdout was correctly not parsed; `production_promotion_eligible=false`; no production behavior changed
